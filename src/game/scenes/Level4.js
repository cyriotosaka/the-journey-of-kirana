...
export class Level4 extends Phaser.Scene {
    ...
    createBackground() {
        ...
        // Swamp atmosphere - murky greens and blues
        if (this.textures.exists('bg_level4_layer1')) {
            this.bgLayer1 = this.add.tileSprite(0, 0, width, height, 'bg_level4_layer1')
                .setOrigin(0).setScrollFactor(0).setDepth(-50).setDisplaySize(width, height);
        } else {
            this.add.rectangle(0, 0, width * 2, height, 0x1a2e2a).setOrigin(0).setScrollFactor(0).setDepth(-50);
        }

        if (this.textures.exists('bg_level4_layer2')) {
            this.bgLayer2 = this.add.tileSprite(0, 0, width, height, 'bg_level4_layer2')
                .setOrigin(0).setScrollFactor(0).setDepth(-40).setDisplaySize(width, height);
        }

        if (this.textures.exists('bg_level4_layer3')) {
            this.bgLayer3 = this.add.tileSprite(0, 0, width, height, 'bg_level4_layer3')
                .setOrigin(0).setScrollFactor(0).setDepth(-30).setDisplaySize(width, height);
        }

        if (this.textures.exists('bg_level4_layer4')) {
            this.bgLayer4 = this.add.tileSprite(0, 0, width, height, 'bg_level4_layer4')
